...
class Subproblem2Solver:
    ...
    def compute_TV(self, a, b, lam, rho):
        ...
        Gg = sum(diffs)
        return 0.5 * (1.0 / math.sqrt(len(a)/2)) * Gg * self.alpha
    ...
    def _run_mergesplit(self, a, b, lam, rho, V_max, seed):
        ...
        G = lambda y: 0.5 * (self.alpha * self.scale * np.abs(y)) / math.sqrt(len(b)/2)
    ...
    def _run_gurobi(self, a, b, lam, rho, V_max, seed):
        ...
        tv_term = 0.5 * gp.quicksum(tv_terms) / math.sqrt(len(b))
