...
class Solver:
    ...
    def compute_Objs(self, a_np, b_np, lam_np, rho, graph, scale):
        ...
        L_form = (rho/2)*inner(a - b + lam, a - b + lam)*self.dx
        TV_form = self.alpha *  sqrt(jump(b)**2 + self.eps_tv) * self.dS

        
        AL_form = J_form + L_form + 0.5 * TV_form
        
        ALhat = assemble(AL_form)

        Jhat = assemble(J_form)

        Lhat = assemble(L_form)
        

        with stop_annotating():
            grads_full_AL = compute_gradient(ALhat, Control(b))
        grads_full_AL = grads_full_AL.vector().get_local()

        tape = get_working_tape()
        tape.clear_tape()
        
        return  float(ALhat), float(Jhat), float(Lhat), grads_full_AL
