...
def run_trial(dim: int, idx: int, params) -> None:
    ...
    if backend == "mergesplit":
        ...
        num_seeds = 1
        print(f"Using chambolle-pock backend: running {num_seeds} random seeds")
    else:
        num_seeds = 10
    ...
    for alpha,rho in zip(alpha_list, rho_list):
        ...
        for (i, seed_i) in enumerate(seeds):
            ...
            while k < params.break_iter and infeas_list[-1] > 1e-1:
                # --- Subproblem 1 ---
                ...
                # print("a",len(a_disc_k1))
                # print("asum", a_disc_k1.sum())
            ...
            # --- Save
            base_dir = f"run_data_admm_periodic_{backend}"
...
def build_argparser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--rho-increase-factor", type=float, default=1.2,
                   help="Multiplicative rho increase factor for periodic updates.")
    return p
