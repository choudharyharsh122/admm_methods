...
def run_trial(dim: int, idx: int, params) -> None:
    ...
    for rho,alpha in zip(rho_list, alpha_list):
        ...
        for (i, seed_i) in enumerate(seeds):
            ...
            sub2 = Subproblem2Solver(mesh, n_x=dim, n_y=2 * dim, alpha=alpha, seed=seed_i)
            ...
            u_list.append(np.zeros(len(f.vector().get_local())))
            ...
            while l < params.break_iter:
                ...
                eta_k = np.linalg.norm(a_k1 - b_k1) ** 2
                ...
                if eta_k <= params.beta * tau_k:
                    ...
                    sub1_obj_k3, compliance_k3, pen_k3, gradL_k3 = core.compute_Objs(a_k, b_k1, lam_k, rho_k, sub2.graph, sub2.scale)
                    ...
                    sub1_obj_2k3, compliance_2k3, pen_2k3, gradL_2k3 = core.compute_Objs(a_k1, b_k1, lam_k, rho_k, sub2.graph, sub2.scale)
                    ...
                    funnel_list.append(params.beta * tau_k)
                    infeas_list.append(np.linalg.norm(a_k1 - b_k1)**2)
                    ...
                    # --- increase penalty and scale dual accordingly
                    # --- please note that we are using the scaled dual form in Augmented Lagrangian
                    rho_k *= 1.25
                    lam_k /= 1.25
